...
const Dashboard = () => {
  ...
  const isProfessor = roles.includes("professor") || roles.includes("assistant") || isAdmin;
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {roles.map((r) => (
                  <span key={r} className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-accent-soft text-accent text-xs font-semibold">
                    ...
                    {r === "super_admin" ? "مدير عام" : r === "dean" ? "عميد الكلية" : r === "professor" ? "عضو هيئة تدريس" : r === "assistant" ? "معيد" : "زائر"}
                  </span>
                ))}
              </div>
            </div>
